    ...
    if request.method == 'POST':
        ...
        if request.form.get('recipient'):
            ...
            end_conversation = Conversation.query.filter(Conversation.user_id == to_user.id,
                                                         Conversation.from_user_id == to_user.id,
            ...
            db.session.commit()
        # delete conversation, request.form[''] would cause a KeyError which cause
        # 400 error
...
def view_conversation(conversation_id):
    ...
    return render_template("messages.html", messages=conversation.messages)


@message.route('/messages/operation', methods=['GET', 'POST'])
def operate_messages():
    pass
